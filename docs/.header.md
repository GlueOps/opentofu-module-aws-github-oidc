...
plus pull_request-triggered runs, and `allowed_subs` replaces it per repo. See [MIGRATION.md](MIGRATION.md) for the v0 -> v1 upgrade and the
...
(`default_branch`, default `"main"`) and by **pull_request-triggered** runs:
